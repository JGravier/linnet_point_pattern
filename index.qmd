...
### Création des objets `linnet` et `lpp` de A à Z et visualisation pour comprendre la structure des objets
...
Il est possible d'étudier un semis de points sur réseau spatial selon les mêmes logiques qu'un semis dans un espace isotrope car diverses mesures ont été adaptées afin de considérer la structure du réseau [voir en particulier @okabe2012].
...
Plusieurs méthodes sont disponibles pour contrôler si le semis de points étudié s'écarte d'une répartition aléatoire, présentant alors une répartition régulière ou une répartition concentrée sur le réseau. 


## Répartitions aléatoires des points sur un réseau spatial et mesures de l'écart aux distributions aléatoires
...
* Un processus aléatoire grâce à `rlpp()`, qui génère $n$ points aléatoires sur le réseau $L$ avec une densité de probabilité spécifié (paramètre `f`). Cette dernière n'e doit'est pas nécessairement être normalisée.
...
# génération de 8 simulations
process_al_8sim <- runiflpp(n = bijoutiers_ppp$n, # nombre de bijoutiers
         L = paris, nsim = 8)
...
# visualisation
par(mfrow=c(2,4))
for (i in 1:length(process_al_8sim)) {
  plot(x = process_al_8sim[[i]], pch = 15, cex = 0.5, main = paste("Simulation ", i))
}
```
